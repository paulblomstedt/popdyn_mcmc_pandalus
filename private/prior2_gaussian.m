function p = prior2_gaussian(varargin)
...
% License.txt, included with the software, for details.
  ...
  ip.addParamValue('mu',0, @(x) isscalar(x) && isreal(x));
...
function lp = prior_gaussian_lp(x, p)
  ...
  lp = 0.5*(-log(2*pi) -log(p.s2)- 1./p.s2 .* sum((x-p.mu).^2,1));
...
function x = prior_gaussian_rnd(p)
% draw sample from the prior
    
  x = p.mu + sqrt(p.s2)*randn;
end    