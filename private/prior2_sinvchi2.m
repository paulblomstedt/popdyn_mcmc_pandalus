function p = prior2_sinvchi2(varargin)
...
function lp = prior_sinvchi2_lp(x, p)
  lp = -((p.nu./2+1) .* log(x) + (p.s2.*p.nu./2./x) + (p.nu/2) .* log(2./(p.s2.*p.nu)) + gammaln(p.nu/2)) ;
...
function x = prior_sinvchi2_rnd(p)
% draw sample from the prior
  x = sinvchi2rand(p.nu,p.s2);
end
