function p = prior2_loggaussian(varargin)
...
% License.txt, included with the software, for details.
  ...
  if init
    ...
    p.fh.rnd = @prior_loggaussian_rnd;
  end
...
function lp = prior_loggaussian_lp(x, p)
  ...
  lp = 0.5*(-log(2*pi) -log(p.s2)- 1./p.s2 .* sum((xt-p.mu).^2,1)) +lJ;
...
function x = prior_loggaussian_rnd(p)
% draw sample from the prior
    
  x = exp( p.mu + sqrt(p.s2).*randn );
end    
